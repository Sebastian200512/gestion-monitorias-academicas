...
export async function POST(req: NextRequest) {
  ...
  // Normalizar fecha a YYYY-MM-DD (por si viene 2025-11-04T00:00:00.000Z)
  try {
    ...
    } else if (/^\d{4}-\d{2}-\d{2}$/.test(fecha_cita) === false) {
      // formato inválido
  ...
  try {
    // 1) Verificar que la disponibilidad es del monitor
    const [dispRows]: any = await conn.execute(
    ...
    // 3) Ver cuántas citas se verían afectadas (pend/confirm) y bloquearlas
    ...
    if (ids.length === 0) {
      ...
      await conn.commit();
      return NextResponse.json({ ok: true, msg: "No había citas por cancelar", count: 0, ids: [] });
    }
    ...
    // Diagnóstico si no coincide el conteo
    if (upd.affectedRows !== ids.length) {
      console.warn("[cancelar-grupo] mismatch afectados", {
        disponibilidad_id,
        fecha_cita,
    ...
    console.error("Error en cancelar-grupo:", error);
